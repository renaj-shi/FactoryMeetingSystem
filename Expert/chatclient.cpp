#include "tcpclient.h"      // 必须放在前面，保证有完整类型
...
void ChatClient::sendText(const QString& orderId, const QString& text) {
    if (!tcp_ || !tcp_->isConnected()) { emit sendFailed(tr("未连接服务器")); return; }
    if (!auth_ || auth_->userId().isEmpty()) { emit sendFailed(tr("未登录")); return; }
    if (orderId.isEmpty()) { emit sendFailed(tr("未加入工单")); return; }
    const QString safe = sanitizeText(text.trimmed());
    if (safe.isEmpty()) return;
    tcp_->sendLine(QString("CHAT|SEND|%1|%2|%3").arg(orderId, auth_->userId(), safe));
}
...
void ChatClient::onLine(const QString& line) {
    ...
    if (line.mid(p0+1, p1-(p0+1)) != "MSG") return;
