/****************************************************************************
** Meta object code from reading C++ file 'workordermanager.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../workordermanager.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'workordermanager.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_WorkOrderManager_t {
    QByteArrayData data[10];
    char stringdata0[132];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_WorkOrderManager_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_WorkOrderManager_t qt_meta_stringdata_WorkOrderManager = {
    {
QT_MOC_LITERAL(0, 0, 16), // "WorkOrderManager"
QT_MOC_LITERAL(1, 17, 16), // "workOrderCreated"
QT_MOC_LITERAL(2, 34, 0), // ""
QT_MOC_LITERAL(3, 35, 10), // "WorkOrder*"
QT_MOC_LITERAL(4, 46, 9), // "workOrder"
QT_MOC_LITERAL(5, 56, 16), // "workOrderUpdated"
QT_MOC_LITERAL(6, 73, 22), // "workOrderStatusChanged"
QT_MOC_LITERAL(7, 96, 15), // "WorkOrderStatus"
QT_MOC_LITERAL(8, 112, 9), // "oldStatus"
QT_MOC_LITERAL(9, 122, 9) // "newStatus"

    },
    "WorkOrderManager\0workOrderCreated\0\0"
    "WorkOrder*\0workOrder\0workOrderUpdated\0"
    "workOrderStatusChanged\0WorkOrderStatus\0"
    "oldStatus\0newStatus"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_WorkOrderManager[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x06 /* Public */,
       5,    1,   32,    2, 0x06 /* Public */,
       6,    3,   35,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void, 0x80000000 | 3, 0x80000000 | 7, 0x80000000 | 7,    4,    8,    9,

       0        // eod
};

void WorkOrderManager::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<WorkOrderManager *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->workOrderCreated((*reinterpret_cast< WorkOrder*(*)>(_a[1]))); break;
        case 1: _t->workOrderUpdated((*reinterpret_cast< WorkOrder*(*)>(_a[1]))); break;
        case 2: _t->workOrderStatusChanged((*reinterpret_cast< WorkOrder*(*)>(_a[1])),(*reinterpret_cast< WorkOrderStatus(*)>(_a[2])),(*reinterpret_cast< WorkOrderStatus(*)>(_a[3]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        switch (_id) {
        default: *reinterpret_cast<int*>(_a[0]) = -1; break;
        case 0:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 0:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< WorkOrder* >(); break;
            }
            break;
        case 1:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 0:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< WorkOrder* >(); break;
            }
            break;
        case 2:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 0:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< WorkOrder* >(); break;
            }
            break;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (WorkOrderManager::*)(WorkOrder * );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&WorkOrderManager::workOrderCreated)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (WorkOrderManager::*)(WorkOrder * );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&WorkOrderManager::workOrderUpdated)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (WorkOrderManager::*)(WorkOrder * , WorkOrderStatus , WorkOrderStatus );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&WorkOrderManager::workOrderStatusChanged)) {
                *result = 2;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject WorkOrderManager::staticMetaObject = { {
    QMetaObject::SuperData::link<QObject::staticMetaObject>(),
    qt_meta_stringdata_WorkOrderManager.data,
    qt_meta_data_WorkOrderManager,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *WorkOrderManager::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *WorkOrderManager::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_WorkOrderManager.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int WorkOrderManager::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void WorkOrderManager::workOrderCreated(WorkOrder * _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void WorkOrderManager::workOrderUpdated(WorkOrder * _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void WorkOrderManager::workOrderStatusChanged(WorkOrder * _t1, WorkOrderStatus _t2, WorkOrderStatus _t3)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t2))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t3))) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
